...
import { 
  FaHome, FaFileAlt, FaBullhorn, FaBars, FaSearch, FaBell, FaUser, FaCog, FaSignOutAlt, FaMoon, FaSun
} from 'react-icons/fa';
...
export default function DashboardLayout({ children, role, userName, userDetail }) {
  ...
  return (
    <Flex h="100vh" bg={pageBg} transition={smoothTransition}>
      {/* Desktop Sidebar */}
      <Box w="250px" bg={sidebarBgColor} p={6} display={{ base: 'none', md: 'block' }} h="100vh" position="sticky" top={0}>
        ...
        <Flex bg={navBg} p={4} px={{ base: 4, md: 8 }} align="center" justify="space-between" shadow="sm" position="sticky" top={0} zIndex={10} transition={smoothTransition}>
          <HStack spacing={4}>
            ...
            <InputGroup w={{ md: '400px' }} display={{ base: 'none', md: 'flex' }}>
              ...
              <Input placeholder="Search..." borderRadius="full" bg={searchBg} border="none" color={textColor} _placeholder={{ color: mutedText }} transition={smoothTransition} />
            </InputGroup>
          </HStack>
            ...
            <Menu>
              <MenuButton as={Button} variant="ghost" borderRadius="full" px={2} py={6} _hover={{ bg: hoverBg }} transition={smoothTransition}>
                <Flex align="center" gap={3}>
                  <Box textAlign="right" display={{ base: 'none', lg: 'block' }}>
                    ...
                    <Text fontSize="xs" color={mutedText} textTransform="capitalize">{role}</Text>
                  </Box>
              ...
              <MenuList shadow="lg" border="none" borderRadius="xl" bg={cardBg} borderColor={borderColor} transition={smoothTransition}>
                <Box px={4} py={2} mb={2}>
                  ...
                  <Text fontSize="xs" color={mutedText}>Logged in as {role}</Text>
                </Box>
                <MenuDivider borderColor={borderColor} />
                <MenuItem icon={<FaUser />} color={textColor} onClick={() => navigate(`/${role}/profile`)} bg={cardBg} _hover={{ bg: hoverBg }} transition={smoothTransition}>My Profile</MenuItem>
                <MenuItem icon={<FaCog />} color={textColor} bg={cardBg} _hover={{ bg: hoverBg }} transition={smoothTransition}>Account Settings</MenuItem>
                <MenuDivider borderColor={borderColor} />
                <MenuItem icon={<FaSignOutAlt />} color="red.500" onClick={() => navigate('/')} bg={cardBg} _hover={{ bg: hoverBg }} transition={smoothTransition}>Logout</MenuItem>
              </MenuList>
            </Menu>
          </HStack>
        </Flex>
