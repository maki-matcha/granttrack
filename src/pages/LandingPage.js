...
import {
  Box, Flex, Text, Heading, Button, Image, Container, SimpleGrid, VStack, HStack, Icon, Link as ChakraLink, Badge, IconButton, Drawer, DrawerBody, DrawerHeader, DrawerOverlay, DrawerContent, DrawerCloseButton, useDisclosure
} from '@chakra-ui/react';
import { useNavigate } from 'react-router-dom';
import { FaRegCalendarAlt, FaRegCheckSquare, FaUserGraduate, FaFileAlt, FaRocket, FaChartLine, FaEye, FaDatabase, FaFacebook, FaInstagram, FaTwitter, FaLinkedin, FaBars } from 'react-icons/fa';
...
const Navbar = () => {
  ...
  return (
    <Flex as={motion.nav} initial={{ y: -100 }} animate={{ y: 0 }} transition={{ duration: 0.8, ease: smoothEase }} align="center" justify="space-between" wrap="wrap" padding="1rem 2rem" bg="brand.darkBlue" color="white" position="fixed" w="100%" zIndex={100} shadow="md">
      ...
      <Drawer isOpen={isOpen} placement="right" onClose={onClose}>
        <DrawerOverlay />
        <DrawerContent bg="brand.darkBlue" color="white">
          <DrawerCloseButton />
  ...
  <Container maxW="7xl" pt={{ base: 28, md: 40 }} pb={{ base: 12, md: 20 }} id="home">
    <Flex align="center" direction={{ base: 'column-reverse', md: 'row' }} textAlign={{ base: 'center', md: 'left' }}>
      ...
      <Box as={motion.div} {...slideLeft} flex={1} pr={{ md: 10 }}>
        <Heading as="h1" size={{ base: 'xl', md: '2xl' }} fontWeight="900" lineHeight="1.2" mb={6} color="brand.darkBlue">
        ...
        <HStack spacing={4} justify={{ base: 'center', md: 'flex-start' }}>
          <Button size="lg" bg="brand.darkBlue" color="white" px={{ base: 6, md: 8 }} _hover={{ bg: 'brand.mediumBlue' }}>Apply Now</Button>
...
const Discover = () => (
  <Box bg="gray.50" py={20} id="scholarships" overflow="hidden">
    <Container maxW="7xl">
      <Heading as={motion.h2} {...slideUp} size={{ base: 'lg', md: 'xl' }} mb={12} color="brand.darkBlue" textAlign={{ base: 'center', md: 'left' }}>
      ...
      <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} spacing={8}>
        {scholarshipsData.map((scholarship, index) => (
           ...
           >
             ...
             <Box p={4} mt="auto">
               <Button w="full" bg="white" color="brand.mediumBlue" borderRadius="full" _hover={{ bg: 'gray.100' }}>Read More</Button>
             </Box>
           </Box>
        ))}
      </SimpleGrid>
...
const AboutAndFeatures = () => (
  <Box id="about" bg="brand.mediumBlue" overflow="hidden">
    <Box 
      as={motion.div} {...slideUp}
      bg="white" pt={{ base: 16, md: 20 }} pb={{ base: 24, md: 32 }} textAlign="center" position="relative" zIndex={2} borderBottomRadius={{ base: '50% 40px', md: '50% 120px' }} 
    >
      <Container maxW="4xl">
        <Heading as="h2" size={{ base: 'xl', md: '2xl' }} mb={4} color="brand.darkBlue">About Us</Heading>
...
// 6. Footer Section
const Footer = () => (
  <Box bg="brand.darkBlue" color="gray.300" pt={16} pb={8} id="contact" as={motion.footer} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1.2, ease: "easeInOut" }}>
    <Container maxW="7xl">
...
export default function LandingPage() {
  return (
    <Box bg="white" minH="100vh" scrollBehavior="smooth">
      <Navbar />
      <Hero />
      <Discover />
      <AboutAndFeatures />
